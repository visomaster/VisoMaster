...
from App.Processors.ModelsData import models_dir
from itertools import product as product
from App.Processors.Utils import FaceUtil as faceutil

def run_detect_landmark(self: 'ModelsProcessor', img, bbox, det_kpss, detect_mode='203', score=0.5, from_points=False):
    kpss_5 = []
    kpss = []
    scores = []
    ...
    if detect_mode=='5':
        ...
        kpss_5, kpss, scores = detect_face_landmark_5(self, img, bbox=bbox, det_kpss=det_kpss, from_points=from_points)

    elif detect_mode=='68':
        if not self.models['FaceLandmark68']:
            self.models['FaceLandmark68'] = self.load_model('FaceLandmark68')

        kpss_5, kpss, scores = detect_face_landmark_68(self, img, bbox=bbox, det_kpss=det_kpss, from_points=from_points)
        ...
        kpss_5, kpss, scores = detect_face_landmark_3d68(self, img, bbox=bbox, det_kpss=det_kpss, from_points=from_points)
        ...
        kpss_5, kpss, scores = detect_face_landmark_98(self, img, bbox=bbox, det_kpss=det_kpss, from_points=from_points)

    elif detect_mode=='106':
        if not self.models['FaceLandmark106']:
            self.models['FaceLandmark106'] = self.load_model('FaceLandmark106')

        kpss_5, kpss, scores = detect_face_landmark_106(self, img, bbox=bbox, det_kpss=det_kpss, from_points=from_points)
        ...
        kpss_5, kpss, scores = detect_face_landmark_203(self, img, bbox=bbox, det_kpss=det_kpss, from_points=from_points)
        ...
        kpss_5, kpss, scores = detect_face_landmark_478(self, img, bbox=bbox, det_kpss=det_kpss, from_points=from_points)
...
def detect_face_landmark_5(self: 'ModelsProcessor', img, bbox, det_kpss, from_points=False):
...
def detect_face_landmark_68(self: 'ModelsProcessor', img, bbox, det_kpss, from_points=False):
    if from_points == False:
...
def detect_face_landmark_3d68(self: 'ModelsProcessor', img, bbox, det_kpss, from_points=False):
    if from_points == False:
        w, h = (bbox[2] - bbox[0]), (bbox[3] - bbox[1])
        center = (bbox[2] + bbox[0]) / 2, (bbox[3] + bbox[1]) / 2
        rotate = 0
...
def detect_face_landmark_98(self: 'ModelsProcessor', img, bbox, det_kpss, from_points=False):
    if from_points == False:
...
def detect_face_landmark_106(self: 'ModelsProcessor', img, bbox, det_kpss, from_points=False):
    if from_points == False:
        w, h = (bbox[2] - bbox[0]), (bbox[3] - bbox[1])
        center = (bbox[2] + bbox[0]) / 2, (bbox[3] + bbox[1]) / 2
        rotate = 0
...
def detect_face_landmark_203(self: 'ModelsProcessor', img, bbox, det_kpss, from_points=False):
    if from_points == False:
        w, h = (bbox[2] - bbox[0]), (bbox[3] - bbox[1])
        center = (bbox[2] + bbox[0]) / 2, (bbox[3] + bbox[1]) / 2
        rotate = 0
...
def detect_face_landmark_478(self: 'ModelsProcessor', img, bbox, det_kpss, from_points=False):
    if from_points == False:
        w, h = (bbox[2] - bbox[0]), (bbox[3] - bbox[1])
        center = (bbox[2] + bbox[0]) / 2, (bbox[3] + bbox[1]) / 2
        rotate = 0
