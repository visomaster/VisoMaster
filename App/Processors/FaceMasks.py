...
def apply_occlusion(self: 'ModelsProcessor', img, amount):
...
def run_occluder(self: 'ModelsProcessor', image, output):
...
def run_dfl_xseg(self: 'ModelsProcessor', image, output):
...
def apply_face_parser(self: 'ModelsProcessor', img, parameters):
...
def run_faceparser(self: 'ModelsProcessor', image, output):
...
def run_CLIPs(self: 'ModelsProcessor', img, CLIPText, CLIPAmount):
...
def soft_oval_mask(self: 'ModelsProcessor', height, width, center, radius_x, radius_y, feather_radius=None):
    """
...
def restore_mouth(self: 'ModelsProcessor', img_orig, img_swap, kpss_orig, blend_alpha=0.5, feather_radius=10, size_factor=0.5, radius_factor_x=1.0, radius_factor_y=1.0, x_offset=0, y_offset=0):
    """
    ...
    mouth_mask = soft_oval_mask(ymax - ymin, xmax - xmin,
                                        (radius_x, radius_y),
                                        radius_x, radius_y,
...
def restore_eyes(self: 'ModelsProcessor', img_orig, img_swap, kpss_orig, blend_alpha=0.5, feather_radius=10, size_factor=3.5, radius_factor_x=1.0, radius_factor_y=1.0, x_offset=0, y_offset=0, eye_spacing_offset=0):
    """
    ...
    def extract_and_blend_eye(eye_center, radius_x, radius_y, img_orig, img_swap, blend_alpha, feather_radius):
        ...
        eye_mask = soft_oval_mask(ymax - ymin, xmax - xmin,
                                        (radius_x, radius_y),
                                        radius_x, radius_y,
                                        feather_radius).to(img_orig.device)
...
def apply_fake_diff(self: 'ModelsProcessor', swapped_face, original_face, DiffAmount):
