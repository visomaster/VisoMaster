...
def run_recognize_direct(self: 'ModelsProcessor', img, kps, similarity_type='Opal', arcface_model='Inswapper128ArcFace'):
    if not self.models[arcface_model]:
        self.models[arcface_model] = self.load_model(arcface_model)

    if arcface_model == 'CSCSArcFace':
        embedding, cropped_image = recognize_cscs(self, img, kps)
    else:
        embedding, cropped_image = recognize(self, arcface_model, img, kps, similarity_type=similarity_type)

    return embedding, cropped_image
    
def run_recognize(self: 'ModelsProcessor', img, kps, similarity_type='Opal', face_swapper_model='Inswapper128'):
    arcface_model = self.get_arcface_model(face_swapper_model)
    return run_recognize_direct(self, img, kps, similarity_type, arcface_model)

def recognize(self: 'ModelsProcessor', arcface_model, img, face_kps, similarity_type):
...
def preprocess_image_cscs(self: 'ModelsProcessor', img, face_kps):
    tform = trans.SimilarityTransform()
...
def recognize_cscs(self: 'ModelsProcessor', img, face_kps):
    # Usa la funzione di preprocessamento
    img, cropped_image = preprocess_image_cscs(self, img, face_kps)
    ...
    embedding_id = recognize_cscs_id_adapter(self, img, None)
    embedding = embedding + embedding_id

    return embedding, cropped_image

def recognize_cscs_id_adapter(self: 'ModelsProcessor', img, face_kps):
    ...
    if face_kps is not None:
        img, _ = preprocess_image_cscs(self, img, face_kps)
...
def calc_swapper_latent_cscs(self: 'ModelsProcessor', source_embedding):
    latent = source_embedding.reshape((1,-1))
    return latent

def run_swapper_cscs(self: 'ModelsProcessor', image, embedding, output):
...
def calc_inswapper_latent(self: 'ModelsProcessor', source_embedding):
...
def run_inswapper(self: 'ModelsProcessor', image, embedding, output):
    if not self.models['Inswapper128']:
...
def calc_swapper_latent_ghost(self: 'ModelsProcessor', source_embedding):
    latent = source_embedding.reshape((1,-1))

    return latent

def calc_fsis_latent(self: 'ModelsProcessor', source_embedding):
...
def run_fsiswapper(self: 'ModelsProcessor', image, embedding, output):
    if not self.models['FaceStyleInswapper256']:
...
def calc_swapper_latent_simswap512(self: 'ModelsProcessor', source_embedding):
    latent = source_embedding.reshape(1, -1)
    #latent /= np.linalg.norm(latent)
    latent = latent/np.linalg.norm(latent,axis=1,keepdims=True)
    return latent

def run_swapper_simswap512(self: 'ModelsProcessor', image, embedding, output):
...
def run_swapper_ghostface(self: 'ModelsProcessor', image, embedding, output, swapper_model='GhostFace-v2'):
