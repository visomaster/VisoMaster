...
def run_detect(self: 'ModelsProcessor', img, detect_mode='Retinaface', max_num=1, score=0.5, input_size=(512, 512), use_landmark_detection=False, landmark_detect_mode='203', landmark_score=0.5, from_points=False, rotation_angles:list[int]=[0]):
    bboxes = []
    kpss_5 = []
    kpss = []
    ...
    if detect_mode=='Retinaface':
        ...
        bboxes, kpss_5, kpss = detect_retinaface(self, img, max_num=max_num, score=score, input_size=input_size, use_landmark_detection=use_landmark_detection, landmark_detect_mode=landmark_detect_mode, landmark_score=landmark_score, from_points=from_points, rotation_angles=rotation_angles)

    elif detect_mode=='SCRFD':
        if not self.models['SCRFD2.5g']:
            self.models['SCRFD2.5g'] = self.load_model('SCRFD2.5g')

        bboxes, kpss_5, kpss = detect_scrdf(self, img, max_num=max_num, score=score, input_size=input_size, use_landmark_detection=use_landmark_detection, landmark_detect_mode=landmark_detect_mode, landmark_score=landmark_score, from_points=from_points, rotation_angles=rotation_angles)

    elif detect_mode=='Yolov8':
        if not self.models['YoloFace8n']:
            self.models['YoloFace8n'] = self.load_model('YoloFace8n')

        bboxes, kpss_5, kpss = detect_yoloface(self, img, max_num=max_num, score=score, use_landmark_detection=use_landmark_detection, landmark_detect_mode=landmark_detect_mode, landmark_score=landmark_score, from_points=from_points, rotation_angles=rotation_angles)

    elif detect_mode=='Yunet':
        if not self.models['YunetN']:
            self.models['YunetN'] = self.load_model('YunetN')

        bboxes, kpss_5, kpss = detect_yunet(self, img, max_num=max_num, score=score, use_landmark_detection=use_landmark_detection, landmark_detect_mode=landmark_detect_mode, landmark_score=landmark_score, from_points=from_points, rotation_angles=rotation_angles)

    return bboxes, kpss_5, kpss

def detect_retinaface(self:'ModelsProcessor', img, max_num, score, input_size, use_landmark_detection, landmark_detect_mode, landmark_score, from_points, rotation_angles:list[int]=[0]):
...
def detect_scrdf(self: 'ModelsProcessor', img, max_num, score, input_size, use_landmark_detection, landmark_detect_mode, landmark_score, from_points, rotation_angles:list[int]=[0]):
    if use_landmark_detection:
        img_landmark = img.clone()
...
def detect_yoloface(self: 'ModelsProcessor', img, max_num, score, use_landmark_detection, landmark_detect_mode, landmark_score, from_points, rotation_angles:list[int]=[0]):
    if use_landmark_detection:
        img_landmark = img.clone()
...
def detect_yunet(self: 'ModelsProcessor', img, max_num, score, use_landmark_detection, landmark_detect_mode, landmark_score, from_points, rotation_angles:list[int]=[0]):
    if use_landmark_detection:
        img_landmark = img.clone()
